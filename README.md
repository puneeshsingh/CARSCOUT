...
- `evals/` — Week 4 eval suite: `cases.py` (test cases), `checks.py` (code-based assertions), `run_evals.py` (runner), `results/*.json` (saved runs), `taxonomy.md` (failure taxonomy), `eval_dashboard.py` (standalone Streamlit viewer - `uv run streamlit run evals/eval_dashboard.py` - not part of the main app's nav; a dev tool, not something a used-car buyer needs to see).
...
- Live URL: shared separately (not published in this repo)
